...
- Epic 10 delivers service-map, pipeline, and ML-infrastructure simulators with controlled actions, objective assertions, and explained state changes.
- Epic 11 delivers the twelve-mission vertical slice, process-table terminal commands, commit checkout recovery, and offline fallback dialogue for online missions.
- Epic 12 delivers the Section 11 public campaign chapters (intro/practice/capstone per domain), prerequisite graph checks, and Black Vault defensive review.
- Epic 13 delivers release readiness: privacy policy, local/cloud data export and deletion, opt-in crash reporting stub, store/ops docs, and security/accessibility assessments.
- Do not publish to stores or production without explicit human approval.
